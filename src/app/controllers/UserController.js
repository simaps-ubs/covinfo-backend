...
class UserController {
  async store(req, res) {
    ...
    await new UserService().create(req).catch(err => res.status(err.statusCode).json({ message: err.message }));
    return res
      .status(200)
  ...
  async update(req, res) {
    ...
    await new UserService().update(req).catch(err => res.status(err.statusCode).json({ message: err.message }));
    return res
      .status(200)
